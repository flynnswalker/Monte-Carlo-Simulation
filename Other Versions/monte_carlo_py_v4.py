...
class NCAA_simulation:
    ...
    def simulate_all_brackets(self):
        processed_dfs = [self.sim_round(group) for _, group in self.big_df.groupby('Group')]
        round_32_teams = pd.concat(processed_dfs, ignore_index=True)
        round_32_matchups = [self.generate_midround_matchups(group) for _, group in round_32_teams.groupby('Group')]
        processed_dfs = [self.sim_round(group) for _, group in round_32_matchups.groupby('Group')]
        round_16_teams = pd.concat(processed_dfs, ignore_index=True)
        round_16_matchups = [self.generate_midround_matchups(group) for _, group in round_16_teams.groupby('Group')]
        processed_dfs = [self.sim_round(group) for _, group in round_16_matchups.groupby('Group')]
        round_8_teams = pd.concat(processed_dfs, ignore_index=True)
        round_8_matchups = [self.generate_midround_matchups(group) for _, group in round_8_teams.groupby('Group')]
        processed_dfs = [self.sim_round(group) for _, group in round_8_matchups.groupby('Group')]
        round_4_teams = pd.concat(processed_dfs, ignore_index=True)
        round_4_matchups = [self.generate_round_4_matchups(group) for _, group in round_4_teams.groupby('Group')]
        processed_dfs = [self.sim_round(group) for _, group in round_4_matchups.groupby('Group')]
        round_2_teams = pd.concat(processed_dfs, ignore_index=True)
        round_2_matchups = [self.generate_midround_matchups(group) for _, group in round_2_teams.groupby('Group')]
        processed_dfs = [self.sim_round(group) for _, group in round_2_matchups.groupby('Group')]
        winners = pd.concat(processed_dfs, ignore_index=True)
        self.winners.append(winner['Team'][0] for winner, group in winners.groupby('Group'))

    def generate_big_df(self):
        # Create a DataFrame for the 'Group' column
        group_numbers = pd.DataFrame({'Group': np.repeat(np.arange(1, self.num_sims + 1), len(self.round_64_matchups))})
        # Duplicate the original DataFrame using concat
        result_df = pd.concat([self.round_64_matchups] * self.num_sims, ignore_index=True)
        # Concatenate the 'Group' DataFrame with the result DataFrame
        self.big_df = pd.concat([result_df, group_numbers], axis=1)  

    def aggregate_simulations(self):
        sagarins = self.sagarins
        self.generate_round_64_matchups(sagarins)
        self.generate_big_df()
        self.simulate_all_brackets()
